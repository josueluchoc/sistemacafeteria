...
import { Mail, Lock, Loader2, Coffee, User, CheckSquare, Square } from 'lucide-react';
...
export default function Login() {
    ...
    const [rememberMe, setRememberMeState] = useState(true); // Renamed setter to avoid conflict with import
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isSignUp) {
                ...
                // 1. Configurar Persistencia ANTES de iniciar sesión
                // Local = Persistente (No se cierra al cerrar navegador)
                // Session = Volátil (Se cierra al cerrar pestaña)
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-gray-100 to-gray-200">
            <div className="w-full max-w-md bg-white/80 backdrop-blur-md border border-white/50 shadow-2xl rounded-3xl p-8 transition-all duration-300">
                ...
                <form onSubmit={handleAuth} className="space-y-4">
                    ...
                    {/* CHECKBOX: Mantener Sesión (Solo visible en Login) */}
                    {!isSignUp && (
                        <div className="flex items-center gap-2 pt-1 pb-1 ml-1 cursor-pointer w-fit" onClick={() => setRememberMeState(!rememberMe)}>
                            {rememberMe ? (
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-blue-500/30 transition-all flex items-center justify-center gap-2 active:scale-95 mt-4"
                    >
